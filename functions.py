...
def reverse_run_length_decoding(run_lengths, amplitudes):
    # Initialize an empty list to store the decoded coefficients
    decoded_coefficients = []

    # Loop through the run-length and amplitude data
    for run_length, amplitude in zip(run_lengths, amplitudes):
        # If run_length is zero, it means there are run_length consecutive zeros
        if run_length == 0:
            decoded_coefficients.extend([0] * amplitude)
        else:
            # Append the run of zeros followed by the amplitude value
            decoded_coefficients.extend([0] * run_length)
            decoded_coefficients.append(amplitude)
